  ...
  *** Interrupt mode IO operation ***
  ===================================
    ...
    (#) If needed, reconfigure and change the input pixel format value, the frame rate
        ...
        Multilive value and Limit value using respectively
                                               ...
                                               const DCMIPP_ParallelConfTypeDef *pParallelConfig)
{
  ...
  else
  {
    ...
    if (pParallelConfig->SynchroMode == DCMIPP_SYNCHRO_EMBEDDED)
    {
      ...
      prescr_reg = (pParallelConfig->SynchroCodes.FrameEndCode << DCMIPP_PRESCR_FEC_Pos)   | \
                   (pParallelConfig->SynchroCodes.LineEndCode << DCMIPP_PRESCR_LEC_Pos)    | \
                   (pParallelConfig->SynchroCodes.FrameStartCode << DCMIPP_PRESCR_FSC_Pos) | \
                   (pParallelConfig->SynchroCodes.LineStartCode << DCMIPP_PRESCR_LSC_Pos);
                                             ...
                                             const DCMIPP_IPPlugConfTypeDef *pIPPlugConfig)
{
  ...
  if (phdcmipp->State != HAL_DCMIPP_STATE_RESET)
  {
    ...
    if (pIPPlugConfig->Client == DCMIPP_CLIENT1)
    {
      ...
      phdcmipp->Instance->IPC1R1 = (pIPPlugConfig->Traffic | pIPPlugConfig->MaxOutstandingTransactions);

      /* Set End word and Start Word of the FIFO of the Clientx */
      phdcmipp->Instance->IPC1R3 = ((pIPPlugConfig->DPREGStart << DCMIPP_IPC1R3_DPREGSTART_Pos) |
                                    (pIPPlugConfig->DPREGEnd << DCMIPP_IPC1R3_DPREGEND_Pos));
      /* Set End word and Start Word of the FIFO of the Clientx */
      phdcmipp->Instance->IPC1R2 = (pIPPlugConfig->WLRURatio << DCMIPP_IPC1R2_WLRU_Pos);
                                        ...
                                        uint32_t CaptureMode)
{
  ...
  if (phdcmipp->PipeState[0]  != HAL_DCMIPP_PIPE_STATE_READY)
  {
    return HAL_ERROR;
  }
  ...
  if (Pipe == DCMIPP_PIPE0)
  {
    ...
    if (phdcmipp->PipeState[0] == HAL_DCMIPP_PIPE_STATE_BUSY)
    {
      ...
      do
      {
        ...
        {
          /* Update error code */
          phdcmipp->ErrorCode |= HAL_DCMIPP_STATE_ERROR;
...
/**
  ...
  * @param  phdcmipp pointer to a DCMIPP_HandleTypeDef structure
  * @param pCropConf pointer to DCMIPP_CropConfTypeDef structure that contains
...
HAL_StatusTypeDef HAL_DCMIPP_PIPE_SetCropConfig(DCMIPP_HandleTypeDef *phdcmipp, uint32_t Pipe,
                                                const DCMIPP_CropConfTypeDef *pCropConf)
{
  assert_param(IS_DCMIPP_PIPE(Pipe));
  assert_param(IS_DCMIPP_PIPE_CROP_AREA(pCropConf->PipeArea));
  assert_param(IS_DCMIPP_PIPE_CROP_HSIZE(pCropConf->HStart));
  assert_param(IS_DCMIPP_PIPE_CROP_HSTART(pCropConf->HSize));
  assert_param(IS_DCMIPP_PIPE_CROP_VSIZE(pCropConf->VSize));
  assert_param(IS_DCMIPP_PIPE_CROP_VSTART(pCropConf->VStart));
  ...
  if (Pipe == DCMIPP_PIPE0)
  {
    ...
    if (((phdcmipp->Instance->P0SCSZR & DCMIPP_P0SCSZR_ENABLE_Msk) != DCMIPP_P0SCSZR_ENABLE) ||
        ((phdcmipp->Instance->PRCR & DCMIPP_PRCR_FORMAT_Msk) == DCMIPP_FORMAT_BYTE))
    {
      ...
      MODIFY_REG(phdcmipp->Instance->P0SCSTR, DCMIPP_P0SCSTR_HSTART | DCMIPP_P0SCSTR_VSTART,
                 (pCropConf->HStart << DCMIPP_P0SCSTR_HSTART_Pos) | (pCropConf->VStart << DCMIPP_P0SCSTR_VSTART_Pos));
      ...
      MODIFY_REG(phdcmipp->Instance->P0SCSZR, DCMIPP_P0SCSZR_HSIZE | DCMIPP_P0SCSZR_VSIZE | DCMIPP_P0SCSZR_POSNEG,
                 (pCropConf->HSize << DCMIPP_P0SCSZR_HSIZE_Pos) | (pCropConf->VSize << DCMIPP_P0SCSZR_VSIZE_Pos) |
                 (pCropConf->PipeArea));
    }
    else
    {
                                                           ...
                                                           uint32_t SelectStart, uint32_t SelectMode)
{
  ...
  if (Pipe == DCMIPP_PIPE0)
  {
    ...
    if (((SelectStart  == DCMIPP_OEBS_EVEN) && (SelectMode > DCMIPP_BSM_ALL)) || (SelectMode > DCMIPP_BSM_DATA_OUT_2) &&
        ((phdcmipp->Instance->PRCR & DCMIPP_PRCR_EDM_Msk) != DCMIPP_INTERFACE_8BITS))
    {
      return HAL_ERROR;
    }
    else
...
HAL_StatusTypeDef HAL_DCMIPP_SetSyncUnmask(DCMIPP_HandleTypeDef *phdcmipp,
                                           DCMIPP_EmbeddedSyncUnmaskTypeDef *SyncUnmask)
{
  uint32_t presur_reg;
  ...
  if (phdcmipp->State == HAL_DCMIPP_STATE_READY)
  {
    ...
    if ((phdcmipp->Instance->PRCR & DCMIPP_PRCR_ESS_Msk) == DCMIPP_SYNCHRO_EMBEDDED)
    {
      ...
      presur_reg = ((SyncUnmask->FrameStartUnmask << DCMIPP_PRESUR_FSU_Pos) | \
                    (SyncUnmask->LineStartUnmask << DCMIPP_PRESUR_LSU_Pos)  | \
                    (SyncUnmask->LineEndUnmask << DCMIPP_PRESUR_LEU_Pos)    | \
                    (SyncUnmask->FrameEndUnmask << DCMIPP_PRESUR_FEU_Pos));
...
/**
  ...
  * @retval HAL state
  */
HAL_DCMIPP_PipeStateTypeDef HAL_DCMIPP_PIPE_GetState(DCMIPP_HandleTypeDef *phdcmipp, uint32_t Pipe)
{
  /* Check Parameters */
  assert_param(IS_DCMIPP_PIPE(Pipe));
