...
int psram_test(void)
{
    ...
    {
#if SDRAM_DATA_WIDTH == 8
        *(__IO uint8_t *)(PSRAM_BANK_ADDR + i * data_width) = (uint8_t)0x5A;
#elif SDRAM_DATA_WIDTH == 16
        *(__IO uint16_t *)(PSRAM_BANK_ADDR + i * data_width) = (uint16_t)0x5AA5;
#else
        *(__IO uint32_t *)(PSRAM_BANK_ADDR + i * data_width) = (uint32_t)0x5AA55AA5;
#endif
    }
...
MSH_CMD_EXPORT(psram_test, psram test)
